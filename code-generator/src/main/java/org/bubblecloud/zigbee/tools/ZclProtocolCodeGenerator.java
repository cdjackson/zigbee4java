...
public class ZclProtocolCodeGenerator {
    ...
    public static void main(final String[] args) {
        ...
        if (!definitionFileZcl.exists()) {
            ...
            try {
                ...
                System.out.println("Reading lines from Zcl definition file failed: "
                        + definitionFileZcl.getAbsolutePath());
                e.printStackTrace();
            }
        }
    ...
    private static void generateZclProfileTypeEnumeration(Context context, String packageRootPrefix, File sourceRootPath)
            throws IOException {
    ...
    private static void generateZclClusterTypeEnumeration(Context context, String packageRootPrefix, File sourceRootPath)
            throws IOException {
    ...
    private static void generateZclCommandTypeEnumeration(Context context, String packageRootPrefix, File sourceRootPath)
            throws IOException {
                ...
                for (final Attribute attribute : cluster.attributes.values()) {
                    if (attribute.attributeAccess.toLowerCase().contains("write")) {
                        ...
                        out.println("    public Future<CommandResult> set" + attribute.nameUpperCamelCase
                                + "(final Object value) {");
                        out.println("        return write(" + attribute.enumName + ", ZclDataType."
                    ...
                    if (attribute.attributeAccess.toLowerCase().contains("read")
                        ...
                        out.println("    public Future<CommandResult> config"
                                + attribute.nameUpperCamelCase
        ...
        for (final Profile profile : profiles) {
            ...
            for (final Cluster cluster : clusters) {
                ...
                for (final Command command : commands) {
                    ...
                    out.println();
                    out.println("    @Override");
                    out.println("    public ZclCommandMessage toCommandMessage() { // TODO: Change to ZdoCommandMessage?");
                    out.println("        final ZclCommandMessage message = super.toCommandMessage();");
                    for (final Field field : fields) {
                        out.println("        message.getFields().put(ZclFieldType." + field.fieldType + ","
                                + field.nameLowerCamelCase + ");");
